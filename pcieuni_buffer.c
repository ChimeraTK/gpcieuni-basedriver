 ...
 *
 * This function attempts to release all the resources held by DMA bufffers in a safe manner. It uses the
 * pcieuni_buffer_list::shutDownFlag flag to disable other threads from using the buffers. If any bufffer appears to be
 * in use this function will wait for up to 1 second for operations on buffer to complete and then move on. If buffer is
 * stuck in a busy mode it's memory won't be released (so memory will leak in this case).
 * @note This function is thread-safe.
