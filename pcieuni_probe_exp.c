...
int pcieuni_probe_exp(struct pci_dev* dev, const struct pci_device_id* id, struct file_operations* pcieuni_fops,
    pcieuni_cdev* pcieuni_cdev_p, char* dev_name, int* brd_num) {
  int m_brdNum = 0;
  int err = 0;
  int tmp_info = 0;
  ...
  *brd_num = m_brdNum;
  ...
  if((err = pci_enable_device(dev))) return err;
  ...
  if(pcieuni_cdev_p->pcieuni_dev_m[m_brdNum]->dev_sts) {
    ...
    device_destroy(
        pcieuni_cdev_p->pcieuni_class, MKDEV(pcieuni_cdev_p->PCIEUNI_MAJOR, pcieuni_cdev_p->PCIEUNI_MINOR + m_brdNum));
  }
